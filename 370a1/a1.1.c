...
/* Merge sort the data using two threads*/
void merge_sort_two_threads(struct block *block)
{
    if (block->size > SPLIT)
    {
        struct block left_block;
        ...
        right_block.size = block->size - left_block.size; // left_block.size + (block->size % 2);
        right_block.data = block->data + left_block.size;
        // create a new thread to merge sort left, use current thread to sort right
        pthread_t t1;
        pthread_create(&t1, NULL, (void *)&merge_sort, (void *)&left_block);
        merge_sort(&right_block);
