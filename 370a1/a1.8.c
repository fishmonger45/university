...
volatile int current_processes_amount = 1;
volatile int *ptr_current_processes_amount = &current_processes_amount;
...
void merge_sort_eight_processes_shared_memory(struct block *block)
{
    if (block->size > SPLIT)
    {
        ...
        if (*ptr_current_processes_amount >= MAXIMUM_PROCESSES)
        {
            merge_sort(&left_block);
            merge_sort(&right_block);
            merge(&left_block, &right_block);
        ...
        else
        {
            ...
            *ptr_current_processes_amount += 1;
            printf("Created process: %d (main) \n", *ptr_current_processes_amount);
