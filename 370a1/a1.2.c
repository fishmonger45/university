...
/* Merge sort the data spawning as many threads as are avaliable*/
volatile int current_thread_amount = 1;
void merge_sort_many_threads(struct block *block)
{
    if (block->size > SPLIT)
    {
        struct block left_block;
        ...
        right_block.data = block->data + left_block.size;
        pthread_t thread;
        int err1 = pthread_create(&thread, NULL, (void *)&merge_sort_many_threads, (void *)&left_block);
        current_thread_amount += 1;
        // Could not create thread, perform both sides of merge here
        if (err1)
        {
        ...
        // Otherwise perform the right side of the sort on the current thread and join
        else
        {
            printf("Creating thread %d\n", current_thread_amount);

            merge_sort(&right_block);
            // Wait for the other thread to be finished
