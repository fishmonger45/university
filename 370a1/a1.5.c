...
/* Merge sort the data using two processes*/
void merge_sort_two_processes_shared_memory(struct block *block)
{
    if (block->size > SPLIT)
    {
        struct block left_block;
        ...
        if (pid != 0)
        {
            ...
            for (int i = 0; i < other_block.size; ++i)
            {
                read(*read_fd, &other_block.data[i], sizeof(int));
            }
        ...
        else
        {
            ...
            // write data
            // Obviously this is going to be slow because you're creating a load of stack frames with /write/ and also calling /read/ loads.
            // Function calls are slow. An optimisation is to write the pipe full (64KB) at a time. But I don't have to make that optimization.
            // IE: this works.
...
int main(int argc, char *argv[])
{
    ...
    merge_sort_two_processes_shared_memory(&block);
