...
/* Merge sort the data using two processes and some shared memory*/
void merge_sort_two_processes_shared_memory(struct block *block)
{
    if (block->size > SPLIT)
    {
        ...
        {
            merge_sort(&left_block);
            // wait for right block to finish in other process
            wait(NULL);
            // merge blocks
            merge(&left_block, &right_block);
        }
...
int main(int argc, char *argv[])
{
    ...
    block.data = mmap(NULL, sizeof(int)*block.size, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_SHARED, -1, 0);
    if (block.data == NULL)
    {
    ...
    // Free the shared block. Must be done this way because freeing shared memory is special
