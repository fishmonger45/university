...
volatile int current_thread_amount = 1;
int MAXIMUM_THREADS = 8;
void merge_sort_eight_threads(struct block *block)
{
    if (block->size > SPLIT)
    {
        struct block left_block;
        ...
        // maximum amount of threads exceeded. Do sort and merge on current thread
        if (current_thread_amount >= MAXIMUM_THREADS)
        {
            printf("Out of threads to allocate, performing complete block mergesort on current thread\n");
            merge_sort(&left_block);
            merge_sort(&right_block);
            merge(&left_block, &right_block);
        }
        ...
        {
            // allocate new thread to left block
            pthread_t thread;
            current_thread_amount += 1;
            printf("Creating thread: %d\n", current_thread_amount);
            pthread_create(&thread, NULL, (void *)&merge_sort_eight_threads, (void *)&left_block);

            // allocate current thread to right block
            merge_sort(&right_block);

            // join and merge on this thread
...
int main(int argc, char *argv[])
{
    ...
    merge_sort_eight_threads(&block);
